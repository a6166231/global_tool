...
type ShortCutItemTrigger = {
    key: number[],
    keys: number,
    call?: Function,
    strict?: boolean,
    next?: ShortCutItemTrigger,
    last?: ShortCutItemTrigger,
}
 ...
 * 键盘数据监听控制器
 */
export class inputListenerControll {
    private _keyMap: Record<number, boolean> = {}
    private _shortCutMap: Record<number, ShortCutItemTrigger> = []

    private _bBubbling: boolean = true

    /**
     * 初始化所有监听数据  会覆盖前一次的on数据
     * @param data 
     * @param bBubbling 是否需要冒泡
     */
    on(data: ShortCutData[], bBubbling: boolean = true) {
        this.clearShortCut()
        this._bBubbling = bBubbling;
        this.initShortCut(data)
        this.addListener()
    }
    ...
    private initShortCut(data: ShortCutData[]) {
        for (let comb of data) {
            for (let i = 0; i < comb.key.length; i++) {
                let key = comb.key[i];
                if (!comb.strict || i == 0) {//非严格顺序
                    this._shortCutMap[key] = { keys: comb.key, key: key, call: comb.call, strict: comb.strict }
                } else {
                    const last = this._shortCutMap[comb.key[i - 1]]
                    this._shortCutMap[key] = { keys: comb.key, key: key, call: comb.call, strict: true, last }
                    last.next = this._shortCutMap[key]
                }
            }
        }
    }
    ...
    private addListener() {
        cc.systemEvent.on(cc.SystemEventType.KEY_DOWN, this.keyDown, this)
        cc.systemEvent.on(cc.SystemEventType.KEY_UP, this.keyUp, this)
    }
    private offListener() {
        cc.systemEvent.off(cc.SystemEventType.KEY_DOWN, this.keyDown, this)
        cc.systemEvent.off(cc.SystemEventType.KEY_UP, this.keyUp, this)
    }

    /** 处理冒泡 */
    private checkBubbling(ev) {
        if (this._bBubbling) return
        ev.propagationStopped = true
        ev.propagationImmediateStopped = true
    }

    private keyDown(ev) {
        this.checkBubbling(ev)
        this._keyMap[ev.keyCode] = true
        this.triggerKey(ev.keyCode)
    }
    private keyUp(ev) {
        this.checkBubbling(ev)
        delete this._keyMap[ev.keyCode]
    }

    private triggerKey(key: number) {
        if (!this._keyMap[key]) return
        let item = this._shortCutMap[key]
        if (!item) return

        if (item.strict) {
            //有前置 前置未激活
            if (item.last && !this._keyMap[item.last.key]) {
